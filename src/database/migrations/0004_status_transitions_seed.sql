-- ═══════════════════════════════════════════════════════════════════════════
-- 0004 — Isi `status_transitions`: state machine sebagai data
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Migrasi ini **tidak dihasilkan `drizzle-kit generate`**. Ia dibuat lewat
-- `--custom`, dan isinya disalin dari
-- `src/database/sql/status_transitions_seed.sql`.
--
-- Alasannya sama dengan migrasi 0003: yang ditulis di sini adalah **baris**,
-- bukan bentuk tabel. `drizzle-kit generate` menyusun DDL dari objek tabel yang
-- ia lacak, dan isi baris bukan bagian dari lacakannya. Kalau berkas ini ditulis
-- sebagai hasil generate, generate berikutnya tidak akan tahu ia ada.
--
-- **Sumber kebenarannya adalah `src/database/sql/status_transitions_seed.sql`.**
-- Kalau ada yang perlu diubah, ubah di sana lebih dulu, lalu salin ke sini
-- sebagai migrasi baru. Jangan mengubah berkas ini setelah ia dijalankan (§8.4).
--
-- ─── Kenapa baris ini bagian dari skema, bukan seeder ──────────────────────
--
-- Tanpa satu baris pun di sini, `availableTransitions` selalu kosong dan tidak
-- ada pekerjaan atau permintaan yang bisa berpindah status sama sekali.
-- Aplikasinya menyala, `/health` hijau, dan tidak ada satu pun alur kerja yang
-- bisa dijalankan. Aturan yang ketiadaannya membuat sistem tidak berfungsi
-- adalah bagian dari skema.
--
-- `on conflict do nothing` dipasang supaya migrasi ini aman dijalankan ulang
-- tanpa menggandakan atau menimpa perubahan yang sudah dibuat orang.
