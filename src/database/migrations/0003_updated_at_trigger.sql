-- ═══════════════════════════════════════════════════════════════════════════
-- 0003 — Trigger `updated_at`: dijaga database, bukan hanya aplikasi
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Migrasi ini **tidak dihasilkan `drizzle-kit generate`**. Ia dibuat lewat
-- `--custom`, dan isinya disalin dari `src/database/sql/updated_at_trigger.sql`.
--
-- Alasannya: `drizzle-kit generate` hanya tahu membuat DDL dari objek tabel yang
-- ia lacak, dan trigger bukan salah satunya. Kalau berkas ini ditulis sebagai
-- hasil generate, maka generate berikutnya tidak akan tahu ia ada — dan tidak
-- akan mempertahankannya.
--
-- **Sumber kebenarannya adalah `src/database/sql/updated_at_trigger.sql`.** Kalau
-- ada yang perlu diubah, ubah di sana lebih dulu, lalu salin ke sini sebagai
-- migrasi baru. Jangan mengubah berkas ini setelah ia dijalankan (§8.4).
--
-- ─── Kenapa perlu ───────────────────────────────────────────────────────────
--
...
--
-- Seluruh migrasi ini **idempoten**: yang sudah ada dilewati, yang belum dibuat.
...
-- ───────────────────────────────────────────────────────────────────────────
--
-- Kedua hitungan di bawah ini harus sama. Kalau tidak, ada tabel ber-`updated_at`
-- yang terlewat, dan itu berarti pemasangnya perlu dijalankan ulang:
--
...
--   SELECT public.apply_updated_at_triggers();
--
-- Ini satu-satunya kelemahan bentuk katalog: ia membaca keadaan saat dijalankan,
-- bukan keadaan saat tabelnya dibuat. Yang menjaganya adalah kebiasaan menyebut
-- baris itu di setiap migrasi yang menambah tabel — bukan sesuatu yang bisa
-- ditegakkan database.
